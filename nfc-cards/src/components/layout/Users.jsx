        ...
        <Layout userData={userData} title="User Registry" showTopNavActions={false}>
                ...
                <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-background/80 backdrop-blur-md animate-in fade-in duration-300">
                    <div className="bg-card border border-border w-full max-w-xl rounded-[2.5rem] p-12 shadow-2xl relative overflow-hidden transition-all duration-300">
                        ...
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <div>
                                <h3 className="text-2xl font-black text-foreground capitalize tracking-tight font-['Outfit']">Identity Hub Audit</h3>
                                <p className="text-foreground/80 text-[10px] font-black capitalize tracking-widest">Participant: {editingUser.uid.substring(0, 16)}</p>
                            </div>
                        </div>

                        <form onSubmit={handleFullUpdate} className="space-y-8">
                            <div className="space-y-3">
                                <label className="text-[10px] font-black capitalize tracking-[0.2em] text-foreground/80 ml-1">Architect Designation</label>
                                <input
                                    type="text"
                                    required
                                    className="w-full px-5 py-3.5 rounded-xl bg-white dark:bg-muted/30 border border-gray-200 dark:border-border text-gray-900 dark:text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all font-medium text-sm"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black capitalize tracking-[0.2em] text-muted-foreground ml-1">Network Access Email</label>
                                <input
                                    type="email"
                                    required
                                    className="w-full px-5 py-3.5 rounded-xl bg-white dark:bg-muted/30 border border-gray-200 dark:border-border text-gray-900 dark:text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all font-medium text-sm"
                            ...
                            <div className="space-y-3">
                                <label className="text-[10px] font-black capitalize tracking-[0.2em] text-muted-foreground ml-1">Clearance Protocol</label>
                                <div className="relative">
                                    <select
                                        className="w-full px-5 py-3.5 rounded-xl bg-white dark:bg-muted/30 border border-gray-200 dark:border-border text-gray-900 dark:text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:border-indigo-500 transition-all font-black capitalize tracking-widest text-[10px] appearance-none cursor-pointer"
                                        value={formData.role}
                                    ...
                                    <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-gray-500 dark:text-muted-foreground">
                            ...
                            <div className="flex items-center gap-4 pt-4">
                                <button
                                    type="submit"
                                    disabled={isSaving}
                                    className="flex-1 py-4 rounded-xl bg-indigo-600 text-white font-black capitalize tracking-[0.2em] text-[10px] hover:bg-indigo-700 hover:shadow-xl hover:shadow-indigo-500/20 active:scale-95 transition-all disabled:opacity-50"
                                >{isSaving ? 'Synchronizing...' : 'Commit Credentials'}</button>
                                <button
                                    type="button"
                                    onClick={() => setEditingUser(null)}
                                    className="px-8 py-4 rounded-xl bg-white dark:bg-secondary border border-gray-200 dark:border-border text-gray-700 dark:text-foreground font-black capitalize tracking-[0.2em] text-[10px] hover:bg-gray-50 dark:hover:bg-muted transition-all"
                                >Cancel</button>
                            </div>
                        </form>
                    </div>
            ...
            <header className="mb-12 flex flex-col md:flex-row md:items-end md:justify-between gap-8 p-4">
                <div className="flex-1">
                    ...
                    <h2 className="text-4xl font-bold text-foreground tracking-tight capitalize">Users</h2>
                ...
                <div className="flex items-center gap-3">
                    <div className={`px-4 py-2.5 rounded-xl border border-border font-black text-[10px] capitalize tracking-widest flex items-center gap-2 ${systemStatus === 'SYNCED' ? 'text-emerald-500 bg-emerald-500/5' : 'text-amber-500 bg-amber-500/5'}`}>
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-muted border-b border-border">
                                <th className="px-10 py-6 text-[10px] font-bold capitalize tracking-[0.3em] text-foreground/70">Operational Node</th>
                                <th className="px-10 py-6 text-[10px] font-bold capitalize tracking-[0.3em] text-foreground/70">Organization</th>
                                <th className="px-10 py-6 text-[10px] font-bold capitalize tracking-[0.3em] text-foreground/70">Contact Details</th>
                                <th className="px-10 py-6 text-[10px] font-bold capitalize tracking-[0.3em] text-foreground/70">Clearance Status</th>
                                <th className="px-10 py-6 text-[10px] font-bold capitalize tracking-[0.3em] text-foreground/70">Network Pulse</th>
                                <th className="px-10 py-6 text-[10px] font-bold capitalize tracking-[0.3em] text-foreground/70 text-right">Actions</th>
                            </tr>
                        </thead>
                            ...
                            {loading ? (
                                    ...
                                    <tr key={i} className="animate-pulse border-b border-border/30">
                                        ...
                                        <td className="px-10 py-8">
                                            <div className="flex flex-col gap-2">
                                                <div className="w-24 h-4 bg-muted/60 rounded-full"></div>
                                                <div className="w-16 h-3 bg-muted/60 rounded-full"></div>
                                            </div>
                                        </td>
                                        <td className="px-10 py-8">
                                            <div className="flex flex-col gap-2">
                                                <div className="w-20 h-4 bg-muted/60 rounded-full"></div>
                                ...
                                filteredUsers.map((user) => (
                                    <tr key={user.uid} className="hover:bg-muted/20 transition-all group">
                                        <td className="px-10 py-8">
                                            <div className="flex items-center gap-5">
                                                <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-secondary to-muted border border-border flex items-center justify-center text-foreground font-black capitalize shadow-inner">
                                                    {(user.displayName || user.email || 'U').charAt(0)}
                                                </div>
                                                <div className="flex flex-col">
                                                    <span className="text-foreground font-bold tracking-tight text-base">{user.displayName || 'Anonymous Architect'}</span>
                                                    <span className="text-[11px] text-muted-foreground font-medium opacity-50 truncate max-w-[150px]">{user.email || 'No access email'}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-10 py-8">
                                            <div className="flex flex-col">
                                                <span className="text-foreground font-bold tracking-tight text-sm truncate max-w-[160px]">{user.companyName || 'Unregistered'}</span>
                                                <span className="text-[10px] text-muted-foreground font-medium opacity-50">{user.country || 'Unknown Region'}</span>
                                            </div>
                                        </td>
                                        <td className="px-10 py-8">
                                            <div className="flex flex-col">
                                                <span className="text-foreground font-bold tracking-tight text-sm truncate max-w-[120px]">{user.mobileNumber || 'N/A'}</span>
                                                <span className="text-[10px] text-muted-foreground font-medium opacity-50 truncate max-w-[140px]">{user.website || 'No Portal'}</span>
                                            </div>
                                        </td>
                                        <td className="px-10 py-8">
                                            <span className={`px-5 py-2 rounded-xl text-[9px] font-black capitalize tracking-[0.15em] border transition-all ${user.role === 'Admin'
                                        ...
                                        <td className="px-10 py-8">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className={`text-[10px] font-black capitalize tracking-widest ${user.status !== 'Standby' ? 'text-foreground' : 'text-amber-500'}`}>
                                ...
                                <tr>
                                    <td colSpan="6" className="px-10 py-32 text-center">
                                        <p className="text-muted-foreground font-black capitalize tracking-widest text-xs opacity-50">Network Synchronization Interrupted or No Nodes Found.</p>
                                    </td>
                                </tr>
                            )}
                        </tbody>
